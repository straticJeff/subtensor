...
use sc_client_api::{
	backend::StateBackend as _, Backend, BlockBackend, BlockchainEvents, CallExecutor,
	ExecutorProvider, ProofProvider, StorageProvider,
};
	...
	Block: BlockT + 'static,
{
	...
	fn query_storage_unfiltered(
		...
		// One state snapshot per block, shared by every key. `client.storage()`
		// would take a fresh snapshot per key: a StateDb pin/unpin pair and a
		// local->shared trie cache merge each time, all of which are global
		// write locks. Set SUBTENSOR_BATCH_QUERY_STORAGE=0 for the old path.
		let batch = std::env::var("SUBTENSOR_BATCH_QUERY_STORAGE")
			.map(|v| v != "0")
			.unwrap_or(true);
		...
		for block_hash in &range.hashes {
			...
			let state = if batch {
				Some(self.client.state_at(*block_hash).map_err(|e| {
					client_err(sp_blockchain::Error::Application(Box::new(e)))
				})?)
			} else {
				None
			};
			...
			for key in keys {
				let (has_changed, data) = {
					let curr_data = match &state {
						Some(state) => state
							.storage(&key.0)
							.map_err(|e| {
								client_err(sp_blockchain::Error::Backend(format!(
									"storage read failed: {e}"
								)))
							})?
							.map(StorageData),
						None => self.client.storage(*block_hash, key).map_err(client_err)?,
					};
