...
const DEFAULT_MAX_INSTANCE_COUNT: u32 = 64;

fn env_u32(name: &str, default: u32, max: u32) -> u32 {
	static CACHE: std::sync::OnceLock<std::sync::Mutex<std::collections::HashMap<String, u32>>> =
		std::sync::OnceLock::new();
	let cache = CACHE.get_or_init(|| std::sync::Mutex::new(std::collections::HashMap::new()));
	let mut guard = cache.lock().unwrap_or_else(|e| e.into_inner());
	*guard.entry(name.to_owned()).or_insert_with(|| {
		std::env::var(name)
			.ok()
...
/// Maximum number of runtime instances executing in parallel.
fn max_instance_count() -> u32 {
	env_u32("SUBTENSOR_WASM_MAX_INSTANCES", DEFAULT_MAX_INSTANCE_COUNT, 1024).max(1)
}

/// Bytes of linear memory zeroed with `memset` on deallocation instead of
/// being returned to the OS with `madvise(MADV_DONTNEED)`. Returning memory
/// forces a TLB shootdown across every core running this process and makes the
/// next call re-fault every page it touches.
fn linear_memory_keep_resident() -> usize {
	env_u32("SUBTENSOR_WASM_KEEP_RESIDENT", 0, u32::MAX) as usize
}

/// Pooling slots kept warm (upstream default is 4).
fn max_unused_warm_slots() -> u32 {
	env_u32("SUBTENSOR_WASM_WARM_SLOTS", 4, 1024)
}
...
fn common_config(semantics: &Semantics) -> std::result::Result<wasmtime::Config, WasmError> {
	...
	if use_pooling {
		...
		pooling_config
			.max_unused_warm_slots(max_unused_warm_slots())
			...
			.table_keep_resident(linear_memory_keep_resident().min(1 << 20));
