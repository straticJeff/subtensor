...
impl<H: Hasher> Drop for LocalTrieCache<H> {
	fn drop(&mut self) {
		...
		// An untrusted (RPC) local cache can skip the merge entirely: the merge
		// takes a GLOBAL write lock, and under a burst of concurrent reads every
		// request queues on it at the end of its work. Block import still
		// populates the shared cache, and the local cache has already done its
		// job of sharing nodes between keys within this request.
		if !self.trusted
			&& std::env::var("SUBTENSOR_TRIE_SKIP_RPC_MERGE")
