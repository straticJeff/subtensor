...
pub struct StateDb<BlockHash: Hash, Key: Hash, D: MetaDb> {
	...
	/// `true` when the pruning mode is `ArchiveAll`, in which case `pin`/`unpin`
	/// mutate nothing and the write lock can be skipped entirely.
	archive_all: bool,
	/// Whether the lock-skipping fast path is enabled.
	fast_pin: bool,
}
...
impl<BlockHash: Hash, Key: Hash, D: MetaDb> StateDb<BlockHash, Key, D> {
	...
	pub fn open(
		...
		let state_db =
			StateDb {
				archive_all: matches!(selected_mode, PruningMode::ArchiveAll),
				fast_pin: std::env::var("SUBTENSOR_STATEDB_FAST_PIN")
	...
	where
		F: Fn() -> bool,
	{
		if self.fast_pin && self.archive_all {
			// ArchiveAll: `StateDbSync::pin` is a no-op returning Ok(()).
			return Ok(());
		}
