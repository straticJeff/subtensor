...
fn generate_options(config: &DatabaseConfig) -> Options {
	...
	if config.secondary.is_some() {
		...
		// An archive node has ~130k SST files; the 512 default means a random
		// read usually has to open a file and read its footer/index first.
		let max_open_files = std::env::var("SUBTENSOR_ROCKSDB_MAX_OPEN_FILES")
			.ok()
			.and_then(|v| v.parse::<i32>().ok())
			.unwrap_or(config.max_open_files);
...
fn generate_block_based_options(config: &DatabaseConfig) -> io::Result<BlockBasedOptions> {
	...
	// `--db-cache` sizes the block cache AND the memtables (the state column
	// takes budget*0.9, and write_buffer_size is a quarter of that), so raising
	// it for cache also inflates memtables. This allows setting the block cache
	// on its own.
	let cache_size = std::env::var("SUBTENSOR_ROCKSDB_BLOCK_CACHE_MB")
		.ok()
		.and_then(|v| v.parse::<usize>().ok())
		.map(|mb| mb * 1024 * 1024)
		.unwrap_or_else(|| config.memory_budget() / 3);
	if cache_size == 0 {
